...
인용 가능한 출처: 아이디어 파일(각 안의 파일 내용)과 전사지 모두 직접 인용할 수 있습니다.
1. 아이디어 파일 또는 전사지에 관련 문장이 있으면 반드시 원문을 직접 인용하세요.
   형식: [파일명]: "원문 그대로 발췌" → 따라서 ~한 효과/의미가 있음
   예시(아이디어 파일): [A안_소셜존.txt]: "카페처럼 자유롭게 대화할 수 있는 공간이 필요하다" → 이 안이 소음 허용 공간 분리를 핵심으로 함을 보여줌
   예시(전사지): [인터뷰.txt]: "휴게실 소음 때문에 집중이 안 돼요" → 소음 갈등이 실제로 존재함을 뒷받침
2. 파일 어디에도 해당 근거가 없으면 반드시 앞에 "추가 근거: "를 붙이세요.
