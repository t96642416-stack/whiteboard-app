...
const CLUSTER_SYSTEM_PROMPT = `당신은 아이디어 분류 전문가입니다. 주어진 아이디어들을 2~5개의 의미있는 유형으로 분류하세요.
각 유형은 공통된 주제, 목적, 또는 특성을 기반으로 만들어야 합니다.
...
반드시 아래 JSON 형식으로만 응답하세요:
{
  ...
  "sections": [
    {
      "title": "유형 제목 (10자 이내)",
      "description": "이 유형의 특징 설명 (30자 이내)",
      "color": "#hex색상",
      "ideaIds": ["idea_id_1", "idea_id_2"]
    }
  ]
}

색상은 파스텔톤으로 선택하세요: #fef9c3(노랑), #dcfce7(초록), #dbeafe(파랑), #fce7f3(분홍), #ede9fe(보라), #ffedd5(주황)
모든 아이디어는 반드시 하나의 섹션에 포함되어야 합니다. 아이디어 ID를 정확히 사용하세요.`;

export async function clusterIdeas(ideas: IdeaInput[]): Promise<ClusterResult> {
  const ideasText = ideas
    .map(
      (idea) =>
        `ID: ${idea.id}\n제목: ${idea.title}\n내용: ${idea.content}\n작성자: ${idea.author}`
  ...
  const result = JSON.parse(jsonMatch[0]) as ClusterResult;
  if (!result.agentType) result.agentType = 'cluster';
  return result;
}
