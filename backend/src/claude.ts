...
모든 근거는 반드시 아래 두 형식 중 하나로만 작성하세요. 인용문 뒤에 → 해석/설명을 추가하는 것은 절대 금지입니다. 각 근거는 60자 이내로 작성하세요.
1. 아이디어 파일에 관련 문장이 있으면 원문을 직접 인용하세요. 파일명은 쓰지 마세요.
   ...
   ※ A안/B안/C안/D안 같은 아이디어 라벨을 출처명으로 쓰는 것은 절대 금지입니다. 아이디어에 첨부된 파일 내용을 인용할 때도 절대로 "A안", "B안", "C안" 등을 괄호 안에 넣지 마세요. 파일 안에 실제 출처(기관명·연구명·통계 등)가 있으면 그것만 쓰고, 없으면 괄호를 완전히 생략하세요. 예시: "공간 분리 후 만족도 향상" (X: A안) → "공간 분리 후 만족도 향상" 또는 "공간 분리 후 만족도 향상" (시설관리연구, 2023)
