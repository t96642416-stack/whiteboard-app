...
1. 첨부 파일(아이디어 파일, 전사지)에 관련 내용이 있으면 반드시 원문을 직접 인용하세요.
   형식: [파일명]: "원문 그대로 발췌" → 따라서 ~한 효과/의미가 있음
   예시: [인터뷰A.txt]: "휴게실에서 대화 소리 때문에 집중이 안 돼요" → 소음 갈등이 실제로 발생하고 있음을 보여줌
2. 파일에서 직접 찾을 수 없는 근거는 반드시 앞에 "추가 근거: "를 붙이세요.
   예시: 추가 근거: 공간 분리는 집중도 향상에 효과적임 (일반 원칙)
3. 파일이 없거나 관련 내용이 없는데 그럴듯한 문장을 만들어 채우는 것은 엄격히 금지합니다.
