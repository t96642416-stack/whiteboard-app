...
const EVIDENCE_RULE = `
근거(evidence) 작성 규칙:
- 첨부된 아이디어 파일 또는 전사지에 관련 내용이 있으면 직접 인용하세요. 예: '전사지: "휴게실 소음 때문에 집중이 어렵다"'
- 파일/전사지에 해당 근거가 없으면 일반 근거를 작성하되 반드시 "추가 근거: " 를 앞에 붙이세요. 예: '추가 근거: 공간 분리는 집중도 향상에 효과적임'`;
  ...
  "ideas": [
    {
      ...
      "name": "아이디어 이름",
      "pros": [
        {"point": "핵심 장점", "evidence": "파일/전사지 직접 인용 또는 근거 없음"},
        {"point": "장점 2", "evidence": "파일/전사지 직접 인용 또는 근거 없음"}
      ],
      "cons": [
        {"point": "핵심 단점", "evidence": "파일/전사지 직접 인용 또는 근거 없음"},
        {"point": "단점 2", "evidence": "파일/전사지 직접 인용 또는 근거 없음"}
      ]
    }
  ],
...
전사지가 있으면 아이디어가 도출된 논의 흐름과 맥락을 파악하여 분석에 반영하세요. 한국어로 작성하세요. 모든 설명은 한 문장(30자 이내)으로 간결하게 작성하세요.
${EVIDENCE_RULE}
...
export async function analyzeIdeas(
  ...
  const referenceFilesText = trimmedRefs.length > 0
    ? "\n\n[전사지 / 논의 원본 — 위 아이디어들이 어떤 맥락과 흐름에서 나왔는지를 보여줍니다. 각 아이디어의 배경·의도·논의 과정을 파악하여 분석에 반영하세요. 전사지 자체는 아이디어가 아닙니다]\n" +
      trimmedRefs.map((r) => `\n--- 전사지: ${r.name} ---\n${r.text}`).join("")
    : "";
