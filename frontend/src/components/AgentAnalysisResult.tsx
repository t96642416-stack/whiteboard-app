...
// 관점 탐색형 UI - Q1/Q2/Q3 탐색 질문
const ExploreView: React.FC<{ result: QuestionAnalysis }> = ({ result }) => {
...
// 효과 예측형 UI - A/B 아이디어 카드 + 예상효과 + 유사사례
...
// 속성 분석형 UI - 장단점 2열 비교
const AttributeView: React.FC<{ result: AttributeAnalysis }> = ({ result }) => {
...
// 레거시 EffectView (effect 타입 하위 호환)
...
const AgentAnalysisResultComponent: React.FC<Props> = ({ result }) => {
  switch (result.agentType) {
    ...
    case "question": {
      const r = result as QuestionAnalysis;
      return <><ExploreView result={r} />{r.searchSources && r.searchSources.length > 0 && <SearchSourcesSection sources={r.searchSources} />}</>;
    }
    case "effect": {
    ...
    case "attribute": {
      const r = result as AttributeAnalysis;
      return <><AttributeView result={r} />{r.searchSources && r.searchSources.length > 0 && <SearchSourcesSection sources={r.searchSources} />}</>;
    }
    case "emphasis": {
